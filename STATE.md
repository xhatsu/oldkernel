...
- **Verification & Test Status**:
  ...
  - High-Load CPU Stability Fix: Resolved 24% CPU escalation by eliminating O(N) linear map scans in `flush_oldest()` and flow eviction, replacing with O(1) iterator pops, immediate flow deallocation upon `fl.buf` consumption, TCP FIN/RST cleanup, and shortening TTLs (`FLOW_TTL=15s`, `PENDING_TTL=3s`). Tested under 8,734 requests over 35s with 0.0% sustained CPU.

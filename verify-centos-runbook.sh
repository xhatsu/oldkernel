...
for f in el68-smoke.sh nt-sniff.py nt-ship.py nt-ship-cpp.cpp nt-sniff-cpp.cpp Makefile nt-run-cpp.sh nt-resource-guard.sh nt-supervise.sh install-oldkernel.sh; do
    [ -s "$HERE/$f" ] && pass "kit file present: $f" || bad "kit file missing: $f"
done
sh -n "$HERE/el68-smoke.sh" "$HERE/install-oldkernel.sh" "$HERE/nt-run-cpp.sh" "$HERE/nt-resource-guard.sh" "$HERE/nt-supervise.sh" "$HERE/build-firstrun.sh" || bad "shell syntax"
